...
#endif
{
  ...
  const ivec2 srcInt = ivec2(srcScreen * vec2(UR(scaled)));
  const uint srcId = uint(srcInt.x + srcInt.y * UR(scaled).x);
  const bool srcValid = srcInt.x >= 0 && srcInt.y >= 0 && srcInt.x < UR(scaled).x && srcInt.y < UR(scaled).y;
  ...
  const ivec2 dstInt = ivec2(dstScreen * vec2(UR(scaled)));
  const uint dstId = uint(dstInt.x + dstInt.y * UR(scaled).x);
  const bool dstValid = dstInt.x >= 0 && dstInt.y >= 0 && dstInt.x < UR(scaled).x && dstInt.y < UR(scaled).y;
  ...
  if (srcValid && dstValid) {
    ...
    const bool dstValidDist = (isSurface ? all(lessThan(abs(dstSamplePos.xyz-(dstHitPersp.xyz/dstHitPersp.w)), vec3(1.f/vec2(UR(scaled)), 0.001f))) : true);
    const bool srcValidDist = (isSurface ? all(lessThan(abs(srcSamplePos.xyz-(srcHitPersp.xyz/srcHitPersp.w)), vec3(1.f/vec2(UR(scaled)), 0.001f))) : true) && any(greaterThan(abs(HIT_SRC.origin.xyz), 0.f.xxx)) && (HIT_SRC.origin.w > 0.f || type == 2);
